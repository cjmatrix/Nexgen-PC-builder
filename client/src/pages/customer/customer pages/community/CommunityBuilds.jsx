...
const CommunityBuilds = () => {
  ...
  const { data, isLoading: loading } = useQuery({
    queryKey: ["community-builds", filter, page],
    queryFn: async () => {
      const params = {
        isFeatured: true,
        limit: 8,
  ...
  useGSAP(
    ...
    { scope: containerRef, dependencies: [products, loading, filter] },
  );
  ...
  const getTheme = (type) => {
    ...
    return {
      ...
      icon: <Wrench className="w-3 h-3 md:w-3.5 md:h-3.5 text-blue-600" />,
      text: "Custom Build",
                ...
                return (
                  ...
                  >
                    ...
                    <div className="aspect-4/3 bg-gray-100 relative overflow-hidden">
                      ...
                      {/* Floating Badge */}
                      <div
                        className={`absolute top-2 left-2 md:top-4 md:left-4 z-20 inline-flex items-center gap-1 md:gap-1.5 px-2 py-0.5 md:px-3 md:py-1.5 rounded-full text-[10px] md:text-xs font-extrabold uppercase tracking-wide border shadow-sm backdrop-blur-md ${theme.badge}`}
                      >
                        {theme.icon}
                        {theme.text}
                      </div>
